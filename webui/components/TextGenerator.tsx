...
export default function TextGenerator() {
  ...
  const cleanGeneratedText = (text) => {
    return text
      .replace(/^!/, "") // Remove leading exclamation mark
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const eventSource = new EventSource(
        `/api/stream?prompt=${encodeURIComponent(
          prompt
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-lg">
      <h2 className="text-2xl font-semibold mb-4">
        Distributed LLM Text Generation
      </h2>
      ...
      {loading && (
        <div className="mt-4 flex items-center justify-center">
          ...
          <span className="ml-2">
            Generating text across distributed workers...
          </span>
        </div>
      )}
      ...
      {generatedText && (
        <div className="mt-4">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2">
            <hr className="col-span-2 border-t border-gray-300 my-2" />
            <div className="bg-blue-50 p-2 rounded border border-blue-200">
              <span className="text-sm font-medium">
                Total Generation Time:{" "}
              </span>
              <span className="text-sm ml-1">
                {generationTime.toFixed(2)} seconds
              </span>
            </div>
            ...
            <div className="bg-green-50 p-2 rounded border border-green-200">
              <span className="text-sm font-medium">
                Simulated Delay (Communication Overhead):{" "}
              </span>
              <span className="text-sm ml-1">
                {totalDelay.toFixed(2)} seconds
              </span>
            </div>
          </div>
        </div>
